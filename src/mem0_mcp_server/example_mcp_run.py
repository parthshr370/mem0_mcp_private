"""Minimal Pydantic AI chat client wired to the Mem0 MCP server."""
...
CONFIG_PATH = Path(
    os.getenv("MEM0_MCP_CONFIG_PATH", BASE_DIR / "config.json")
).expanduser()
CONFIG_SERVER_KEY = os.getenv("MEM0_MCP_CONFIG_SERVER", "mem0")
...
def build_agent(server: MCPServerStdio) -> tuple[Agent, str]:
    ...
    system_prompt = (
        ...
        "Default to user_id 'parthshr370' unless the user supplies another value, and inject it into every filter. "
